...
. https://github.com/redhat-ai-services/etx-agentic-ai[Fork the etx-agentic-ai git repository,window=_blank] and make sure each Team member has access to the fork i.e. add them as collaborators.
+
.GitHub Repo Fork
...
[source,bash,options="wrap",role="execute"]
----
chmod 755 bootstrap/pre-commit
cd .git/hooks
ln -s ../../bootstrap/pre-commit pre-commit
----
====
